...
const allUniqueAllergens = food.flatMap(food => food.allergens).filter(unique);

let allergensWithCandidates = allUniqueAllergens.map(allergen => {
  ...
  const candidates = possibleIngredients
    ...
    .filter(unique)
    .filter(ingredient =>
      possibleIngredients.every(possibility => possibility.includes(ingredient))
    );
...
const possibleAllergens = allergensWithCandidates.flatMap(
  allergen => allergen.candidates
);

const definitelyNotAllergens = allIngredients.filter(
  ingredient => !possibleAllergens.includes(ingredient)
);

const appearences = allIngredients.filter(ingredient =>
  definitelyNotAllergens.includes(ingredient)
).length;
...
while (allergensWithCandidates.length) {
  const { allergen, candidates } = allergensWithCandidates.find(
    possibile => possibile.candidates.length === 1
  );
  ...
  allergensWithCandidates = allergensWithCandidates
    ...
    .map(possibile => ({
      ...possibile,
      candidates: possibile.candidates.filter(
