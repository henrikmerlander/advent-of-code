...
const rules = input.slice(0, input.indexOf('your ticket:') - 1).map(raw => {
  const [, field, min1, max1, min2, max2] =
    /([\w ]+): (\d+)-(\d+) or (\d+)-(\d+)/.exec(raw);
...
const passesRule = (value, { min1, max1, min2, max2 }) =>
  (min1 <= value && value <= max1) || (min2 <= value && value <= max2);

const invalidValues = values =>
  values.filter(value => rules.every(rule => !passesRule(value, rule)));
...
const multiplied = orderedColumns.reduce(
  (acc, curr, currIndex) =>
    curr.startsWith('departure') ? (acc *= myTicket[currIndex]) : acc,
  1
);
