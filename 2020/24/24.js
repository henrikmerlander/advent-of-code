...
const list = input.map(raw =>
  raw.match(/[sn]?[ew]/g).map(direction => {
    switch (direction) {
      case 'nw':
        return { x: 0, y: 1, z: -1 };
      case 'se':
...
let floor = list.reduce((floor, instructions) => {
  const tile = instructions.reduce(
    (tile, instruction) => ({
...
const pad = floor =>
  Array.from(floor.keys()).forEach(tile =>
    findAdjacent(JSON.parse(tile))
...
const apply = floor => {
  ...
  return Array.from(floor.entries()).forEach(([tile, isBlack]) => {
    ...
    else if (!isBlack && adjacentBlack === 2) floor.set(tile, true);
  });
};
