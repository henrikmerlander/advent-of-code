...
const applyRules = (seatmap, strategy, occupiedThreshold) =>
  seatmap.map((row, y) =>
    row.map((seat, x) => {
      const occupiedSeats = strategy(seatmap, x, y).filter(
        seat => seat === occupied
      ).length;

      if (seat === empty && occupiedSeats === 0) return occupied;
      else if (seat === occupied && occupiedSeats >= occupiedThreshold)
        return empty;
      else return seat;
    })
  );
...
const occupiedSeats = seatmap =>
  seatmap.flat().filter(seat => seat === occupied).length;
