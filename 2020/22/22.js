...
const score = deck =>
  deck.reduce(
    (acc, curr, currIndex) => acc + curr * (deck.length - currIndex),
    0
  );
...
const combat = (startingDeck1, startingDeck2, recursive) => {
  ...
  while (!isEnd()) {
    ...
    if (previousRounds.has(round)) return { p1Win: true, score: score(deck1) };
...
const recursiveCombat = (startingDeck1, startingDeck2) =>
  combat(startingDeck1, startingDeck2, true);
