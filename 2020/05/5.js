...
const instructionsToBinary = instructions => instructions.replace(/(F|L)/g, 0).replace(/(B|R)/g, 1)

const seatIds = input
  .map(boardingPass => {
    const { groups: { rowInstructions, columnInstructions } } = /(?<rowInstructions>(B|F){7})(?<columnInstructions>(L|R){3})/.exec(boardingPass)
...
const expectedTotal = Array(seatIds.length + 1).fill()
