...
for (let move = 1; move <= 100; move++) {
  const picked = [1, 2, 3]
    .map(() => cups.splice((cups.indexOf(currentCup) + 1) % cups.length, 1)[0])

  const destination = Array.from(cups)
    .sort((a, b) => b - a)
...
const order = cups
  .reduce((acc, _, currIndex) => `${acc}${cups[(currIndex + cups.indexOf(1)) % cups.length]}`, '')
  .replace('1', '')
