...
const totalScore = input.reduce((acc, curr) => {
  const [opponent, you] = curr.replace('X', 'A').replace('Y', 'B').replace('Z', 'C').split(' ')

  return acc += (shapeScore[you] + outcomeScore[abc.indexOf(you)][abc.indexOf(opponent)])
}, 0)
